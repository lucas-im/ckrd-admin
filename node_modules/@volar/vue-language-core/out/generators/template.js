...
function generate(ts, vueCompilerOptions, sourceLang, templateAst, hasScriptSetup, cssScopedClasses = []) {
    ...
    function visitNode(node, parentEl) {
        ...
        if (node.type === 1 /* CompilerDOM.NodeTypes.ELEMENT */) {
            ...
            // {{ ... }}
            writeInterpolation(node.content.loc.source, node.content.loc.start.offset, {
                vueTag: 'template',
                capabilities: capabilitiesSet.all,
            }, '(', ');\n', node.content.loc);
            writeInterpolationVarsExtraCompletion();
            writeFormatCode(node.content.loc.source, node.content.loc.start.offset, formatBrackets.curly);
        }
