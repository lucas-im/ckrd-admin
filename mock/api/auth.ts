...
const apis: MockMethod[] = [
  ...
  // 用户+密码 登录
  {
    url: '/mock/login',
    method: 'post',
    response: async (options: Service.MockOption): Promise<Service.MockServiceResult<ApiAuth.Token | null>> => {
      ...
      const findItem = users.find(item => item.userName === userName && item.password === password);
  ...
  // 获取用户信息(请求头携带token, 根据token获取用户信息)
  {
    ...
    method: 'get',
    response: async (options: Service.MockOption): Promise<Service.MockServiceResult<ApiAuth.UserInfo | null>> => {
      ...
      const users = await getUsers();
      const isInUser = users.some(item => {
  ...
  },
  {
    ...
    response: async (options: Service.MockOption): Promise<Service.MockServiceResult<ApiAuth.Token | null>> => {
      ...
      const users = await getUsers();
      const findItem = users.find(item => item.refreshToken === refreshToken);
